...
if (incremental_flag == '0'): 
    ...
    if spark.catalog.tableExists('cars_catalog.gold.dim_branch'):
        max_value_df = spark.sql("SELECT max(dim_model_key) FROM cars_catalog.gold.dim_branch")
...
# Incremental RUN 
if spark.catalog.tableExists('cars_catalog.gold.dim_branch'):
    delta_table = DeltaTable.forPath(spark, "abfss://gold@datalakecarsale.dfs.core.windows.net/dim_branch")
...
# Initial RUN 
    ...
    df_final.write.format("delta")\
        .mode("overwrite")\
        .option("path", "abfss://gold@datalakecarsale.dfs.core.windows.net/dim_branch")\
